...
template < class T > unsigned int CHashTable < T >::Hash( const char *key )
{
...
template < class T >
  bool CHashTable < T >::FindEntry( const char *key, unsigned int &i,
                                    CHashTableEntry * &p )
{
  i = Hash( key );
...
template < class T > T * CHashTable < T >::Find( const char *key )
{
  unsigned int i;
  CHashTableEntry *p;
  if( FindEntry( key, i, p ) )
    return &( p->value );
  return 0;
}
...
template < class T > bool CHashTable < T >::Delete( const char *key )
{
  unsigned int i;
  CHashTableEntry *p;
  if( !FindEntry( key, i, p ) )
