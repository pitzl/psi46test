...
vector < string > vcnames; // DP, global. how to move into class CInterpreter?
...
bool CCmdLine::read( FILE * f )
{
  ...
  if( isInteractive(  ) )
    fputc( '>', stdout );
  if( fgets( s, CMDLINELENGTH, f ) == NULL )
    return false;
#else
  if( isInteractive(  ) ) {
    char *line = readline( "> " );
  ...
  cmd = s;
  while( isWhitespace( cmd[0] ) )
    cmd++;

  par = cmd;
  while( isAlphaNum( *par ) )
    par++;
  if( par[0] != 0 ) {
    par[0] = 0;
    par++;
...
bool CCmdLine::getIntRange( int &valuemin, int &valuemax, int skipmin,
                            int skipmax )
{ // nn:nn
 ...
 // scan for ":"
  if( par[0] == ':' ) { // scan second number
    par++;
    if( isWhitespace( par[0] ) || ( par[0] == 0 ) )
      vmax = skipmax;
    else {
      if( !getInt( vmax, -10000, 10000 ) )
        return false;
    }
  }
  else
...
void CInterpreter::help(  )
{
  ...
  printf( " quit         quit commander\n" ); // DP

 // cmdList is a CHashTable<CCommand> 
 // class CCommand is in command.h
 // class CCommand has help and function
 // class CHashTable in in htable.h
 /* unsorted (hash table):
    cout << cmdList.GetSize() << " commands:" << endl;
    CCommand* p = cmdList.GetFirst();
    while(p) {
    printf( "%s:  %s\n", cmdList.GetKey(), p->m_help );
    p = cmdList.GetNext();
    }
  */
 // sorted (vector): DP
...
bool CInterpreter::run( FILE * f, int iter )
{
  ...
  while( true ) {
    ...
    if( !cmdline.read( f ) )
      break;
    if( cmdline.getName(  )[0] == 0 )
      continue; // empty line
    if( cmdline.getName(  )[0] == '-' )
      continue; // comment
    if( cmdline.getName(  )[0] == '#' )
      continue; // comment DP
    ...
    if( p )
      p->m_exec( cmdline );
    else if( cmdline.isCmd( "help" ) )
      help(  );
    else if( cmdline.isCmd( "exit" ) )
      break;
    else if( cmdline.isCmd( "quit" ) ) {
      cout << "quit" << endl;
      break;
    }
    // script:
